...
function loadPreset(name = selectedPresetName.value): void {
    const preset = presets.value.find((item) => item.name === name);
    if (!preset) return;
    applyForm(preset.form as GenerationForm);
    selectedPresetName.value = preset.name;
    setMessage(`Preset loaded: ${preset.name}`, "success");
}

function saveCurrentPreset(): void {
    try {
        const preset = savePreset(newPresetName.value, form);
        refreshPresets();
        selectedPresetName.value = preset.name;
        newPresetName.value = "";
        setMessage(`Preset saved: ${preset.name}`, "success");
    } catch (error) {
...
function deleteSelectedPreset(): void {
    ...
    removePreset(selectedPresetName.value);
    const removedName = selectedPresetName.value;
    selectedPresetName.value = "";
    refreshPresets();
    setMessage(`Preset deleted: ${removedName}`, "success");
}

function exportCurrentPresets(): void {
    const blob = new Blob([exportPresets()], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `deep-n-presets-${new Date().toISOString().slice(0, 10)}.json`;
    link.click();
    URL.revokeObjectURL(url);
    setMessage("Presets exported.", "success");
}

function openPresetImport(): void {
...
async function importPresetFile(event: Event): Promise<void> {
    ...
    if (!file) return;
    try {
        const count = importPresets(await file.text());
        refreshPresets();
        setMessage(`${count} preset(s) imported.`, "success");
    } catch (error) {
        setMessage(error instanceof Error ? error.message : String(error), "error");
    }
}
    ...
    <div class="shell">
        ...
        <div v-if="activeTab !== 'settings'" class="layout">
            <section class="panel control-panel">
                ...
                <div class="prompt-card stack-top">
                    <div class="panel-header">
                        <div>
                            <h2 class="panel-title">Presets</h2>
                            <p class="hint">Saved locally in this browser. Auxiliary images are not stored.</p>
                        </div>
                    </div>
                    <div class="fields">
                        <div class="field">
                            <label>Load preset</label>
                            <select v-model="selectedPresetName" @change="loadPreset()">
                        ...
                        <div class="field">
                            ...
                            <input v-model="newPresetName" placeholder="Anime watercolor" @keyup.enter="saveCurrentPreset" />
                        </div>
                    </div>
                    <div class="actions stack-top">
                        <button class="btn-secondary" type="button" @click="saveCurrentPreset">Save current</button>
                        <button class="btn-ghost" type="button" :disabled="!selectedPresetName" @click="deleteSelectedPreset">Delete selected</button>
                        <button class="btn-ghost" type="button" @click="exportCurrentPresets">Export JSON</button>
                        <button class="btn-ghost" type="button" @click="openPresetImport">Import JSON</button>
                        <input ref="presetFileInput" type="file" accept="application/json,.json" hidden @change="importPresetFile" />
                    </div>
                </div>
